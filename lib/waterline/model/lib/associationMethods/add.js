  ...
  async.eachLimit(records, 10, function(association, next) {
    ...
    if(association !== null && typeof association === 'object' && Object.keys(association).length > 0) {
      ...
      if(hasOwnProperty(association, associatedCollection.primaryKey)) {
        return self.updateRecord(associatedCollection, schema, association, next);
      }
...
Add.prototype.updateRecord = function(collection, attribute, values, cb) {
  ...
  if(junctionTable) {
    ...
    if (values !== null && typeof values === 'object' && Object.keys(values).length > 0) {
      var criteria = {};
      criteria[collection.primaryKey] = values[collection.primaryKey];
      collection.update(criteria, values, function (err) {
        if (err) {
          self.failedTransactions.push({
            type: 'update',
            collection: collection.identity,
        ...
        return self.createManyToMany(joinCollection, attribute, values[collection.primaryKey], cb);
      });
    } else {
      return self.createManyToMany(joinCollection, attribute, values, cb);
    }
    ...
    if (values !== null && typeof values === 'object' && Object.keys(values).length > 0) {
      criteria[associationKey] = values[collection.primaryKey];
    } else {
